...
function EmpPage() {
  ...
  const rows = useDb(() => db.all("employees").filter(r => r.organization_id === orgId));
  return <CrudPage title="Employees" subtitle="Manage staff, designations and shifts." exportName="employees"
    table="employees" data={rows} orgId={orgId} defaults={{ status: "active", rating: 4.5 }}
    columns={[
